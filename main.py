...
with st.sidebar:  # 侧边栏 布局
    模型列表 = ["gemini-2.5-flash-preview-04-17","deepseek-V3","kimi_moonshot"]
    单选模型 = st.selectbox("模型选择:", 模型列表, index=0)
    f"你选择的模型为：{单选模型}"  # 显示 字符串  # gemini-2.5-flash-preview-04-17
    st.markdown("---")

    if 单选模型:
        if 单选模型 == "gemini-2.5-flash-preview-04-17":
...
# 行文字 输入框
st_视频主题 = st.text_input("💡请输入创意构思")  # 行文字 输入框  # 返回 文字

# 数字 输入框
st_视频时长 = st.number_input("⌚请输入视频时长（分钟）[每个分镜 1-5 秒]",value=1.0,min_value=0.1,step=0.1)  # 数字 输入框  # 返回 输入数字
...
if st_提交按钮:
    ...
    with st.spinner("AI正在思考中，请等待..."):  # 使用 st.spinner 创建一个加载动画，提示用户 AI 正在思考 # 只要以下代码没有运行完，就会一直提示正在加载
        ...
        if 单选模型 == "gemini-2.5-flash-preview-04-17":
            st_模型 = Model(temperature=st_创造力,api_key=st_api_key).gemini
        elif 单选模型 == "deepseek-V3":
